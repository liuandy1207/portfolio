...
## How It Works
Each level is randomly generated using BFS to guarantee solvability. In the code, the monster is always aware of where you are, so when you make a noise, it can use BFS again to find the shortest path to you and take a step in that direction. Otherwise, it moves randomly. Furthermore, each noise has an intensity that dictates how much through the maze it perforates, so the monster only uses the information that you are close when you make a noise close enough to it. Footsteps were a small intensity, but walking into a wall was a very large intensity. An ESP32 handles the processing, a piezo buzzer handle the noise, joystick for movement, and a tissue box was used to enclosed the device in a presentable manner. 
